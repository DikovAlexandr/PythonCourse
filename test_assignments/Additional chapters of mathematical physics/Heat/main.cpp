...
std::tuple<double, double, double> right_bord_cond(double t) {
    std::tuple<double, double, double> result(3, 1, 5 - 8 * t + 3 * pow(t, 2) + (3 + t * tan(t)) / (2 * cos(t)));
    return result;
}
...
int main() {
    ...
    for (int i = 0; i < SIZE_X; ++i) {
        ...
        for (int j = 0; j < SIZE_T; ++j) {
            ...
            std::vector<double> b(SIZE_X - 1, 0);
            a[0] = - B_0 / A_0;
            b[0] = d[0] / A_0;
            // Прямой прогон
    ...
    std::vector<double> U_0(SIZE_X, 0);
    for (int i = 0; i < SIZE_X; ++i){
    ...
    // Вывод в файл
    std::ofstream fout;
    fout.open(R"(..\\..\\Heat\\solve.csv)");
    for (auto &x: U) {
        fout << std::setprecision(8) << x << " ";
    }
    fout.close();

    fout.open(R"(..\\..\\Heat\\analytical.csv)");
    for (auto &x: U_0) {
        fout << std::setprecision(8) << x << " ";
    }
    fout.close();

    fout.open(R"(..\\..\\Heat\\X.csv)");
    for (auto &x: X) {
        fout << std::setprecision(8) << x << " ";
    }
    fout.close();

//    system("start ..\\..\\Heat\\\\grapher.exe");
